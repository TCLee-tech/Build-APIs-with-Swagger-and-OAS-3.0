...
public class ProductApiController implements ProductApi {
    ...
    public ResponseEntity<Product> productProductIDGet(@Parameter(in = ParameterIn.PATH, description = "", required=true, schema=@Schema()) @PathVariable("productID") Integer productID,@Parameter(in = ParameterIn.HEADER, description = "" ,schema=@Schema()) @RequestHeader(value="custom-security-header", required=false) Integer customSecurityHeader) {
        String accept = request.getHeader("Accept");
        if (accept != null && accept.contains("application/json")) {
            try {
